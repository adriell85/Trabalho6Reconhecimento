...
def plotAccuracyRejectionCurvePerWr(Wr_values, all_accuracy_lists, all_rejection_rate_lists, baseName):
    ...
    for i, Wr in enumerate(Wr_values):
        ...
        plt.plot(avg_rejection_rate, avg_accuracy, marker='o', linestyle='-', label=f'Wr={Wr}')
        plt.annotate(f'Wr={Wr}', (avg_rejection_rate, avg_accuracy))
...
def plotDataPoints(X_train, y_train, X_test, y_test, baseName, iteration, phase, originalLabels):
    ...
    plt.savefig(
        f"Resultados_BayesianGaussianRejectionQuant/{baseName}/Distribuicao_Dados_{baseName}_{phase}_Iteration_{iteration}.png")
    # plt.show()
